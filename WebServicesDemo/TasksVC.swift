...
class TasksVC: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        let addButt = UIBarButtonItem(barButtonSystemItem: .add, target: self, action: #selector(handleAdd))
    ...
    @objc private func handleAdd(){
        let alert = UIAlertController(title: "Add New Task", message: "Enter name", preferredStyle: .alert)
...
extension TasksVC: UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? TaskCell else{ return UITableViewCell()}
        
        let task = tasks[indexPath.row]
        cell.configerCell(task: task)
        return cell
    }
}
...
extension TasksVC: UITableViewDelegate {
    ...
    private func handleEdit(task: Task,indexPath: IndexPath){
        ...
        let alert = UIAlertController(title: "Edit Task", message: "Enter name", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: nil))
        alert.addTextField(configurationHandler: {
            $0.text = task.task
            $0.placeholder = "New Task"
